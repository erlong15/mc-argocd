...
| `nginx-prod` / `podinfo-prod` created с path=`{{path.path}}` (ComparisonError, `repository not accessible`) | template не отработал: в fasttemplate-синтаксисе переменная называется `{{path}}`, а `{{path.path}}` — это уже goTemplate-форма | в `repo/infra/projects/prod/appset.yaml` используется `path: '{{path}}'`; если хочется goTemplate-синтаксис — добавить `spec.goTemplate: true` И заменить ВСЕ подстановки на `{{ .path.path }}`, `{{ .path.basename }}` |
| `nginx-prod` pod в CrashLoopBackOff, init-container `preserve-logs-symlinks`: `exec: /bin/bash: no such file` | bitnami-чарт жёстко завязан на свой layout образа (bash в /bin, entrypoint в /opt/bitnami/scripts/nginx/); с upstream `nginx:alpine` или любым не-bitnami образом init-container падает | в `repo/apps/prod/nginx/values.yaml` образ выставлен `docker.io/bitnamilegacy/nginx:1.28.0` — публичный форк bitnami-образов после миграции в августе 2025. Если чарт меняет требования — поднимать тег или свапать на другой чарт (наш локальный `repo/charts/nginx-demo` умеет любой nginx-образ) |
